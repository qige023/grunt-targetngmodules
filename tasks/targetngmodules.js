...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('targetngmodules', 'Produces angularjs-output depending on grunt release version', function() {
...
//                    contents = contents.replace(new RegExp('<!--[\\[\\(]if target (.*?)[\\]\\)]>(<!-->)?([\\s\\S]*?)(<!--)?<![\\[\\(]endif[\\]\\)]-->', 'g'), function(match, $1, $2, $3) {
                    contents = contents.replace(new RegExp('\/\\*[\\[\\(]if target (.*?)[\\]\\)]([\\S\\s]*?)\\*\/', 'g'),
